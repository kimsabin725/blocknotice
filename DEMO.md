...
**이 문서는 데모데이 현장에서 직접 돌려 보이는 5분 대본이다.** 온라인 제출용 영상(2분 48초)과는
분량도 구성도 다르다. 영상은 완성본 하나로 제출하고, 이 대본은 심사위원 앞에서 명령을 실제로
실행하며 말할 내용이다.
...
> 승인된 요청은 트랜잭션이 되어 체인에 남습니다. 거절된 요청은 아무 데도 남지 않습니다.
> 기관 내부 DB에만 있으니 사유를 나중에 바꿔 적어도, 행을 지워도 밖에서는 알 수 없습니다.
> BlockNotice는 그 판단에 서명 영수증을 붙이고, 기관이 서명한 기한이 지켜졌는지를
> 기관 서버에 한 번도 접속하지 않고 확인합니다. 상환을 승인하겠다는 약속이 아니라
> 기록하고 증빙하겠다는 약속을 다룹니다.
>
> 오늘 보여드릴 장면은 금 RWA 상환입니다. 구간이 셋이라 「누구의 시계에서 멈췄는가」가 가장
> 선명하게 갈리기 때문이고, 에스크로 자체는 소각 가능한 임의의 ERC-20을 받습니다.
>
...
시연 전에는 실제 실행 결과, 리포트, 발표 PDF가 같은 커밋 기준인지 확인한다.
화면에 개인키·환경 파일·개인정보를 노출하지 않는다.
온라인 제출 영상은 2026-09-18에 녹화했고, 뒷부분 78초는 대본 낭독이 아니라 `forge test`·공개 RPC
대조·시나리오 리포트·Sepolia 익스플로러를 실제로 띄운 화면이다.
