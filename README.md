...
**거절된 요청은 트랜잭션이 되지 못합니다.** 승인은 체인에 남지만, 막힌 요청은 기관 내부 DB에만
남습니다. 나중에 기관이 그런 요청은 없었다고 하거나 사유를 바꿔 적어도 요청자에게는 반박할 근거가
없습니다. 지워도 밖에서는 지워진 줄 모릅니다.

BlockNotice는 그 판단에 **서명 영수증**을 붙이고, 기관이 서명한 처리 기한이 지켜졌는지를
**기관 서버에 한 번도 접속하지 않고** 확인하게 합니다.

| 무엇을 보장하는가 | 어떻게 |
|---|---|
| 기록을 바꾸면 탐지된다 | 서명 영수증 + 공개 머클 앵커. 사후 교체는 루트가 어긋납니다 |
| 제3자가 기관 서버 없이 검증한다 | 공개 로그 이벤트만으로 상태 전이를 재구성합니다 |
| 요청자도 기관도 발뺌 못 한다 | EIP-712 서명 요청과 서명 영수증이 양쪽을 함께 묶습니다 |

**안 올린 경우까지 다룹니다.** 누락은 그 자체로 위반이 아니지만, 요청자가 자기 영수증과 기한으로
...
함수·기한·설계 보완은 [상환 사양](docs/redemption-design.md), 작업 상태는
[제출본 현황](docs/submission-status.md)에 있습니다.

### 금 RWA 상환은 주제가 아니라 예시입니다

위 구조를 가장 두껍게 보여주는 장면으로 금 실물 상환을 골랐습니다. 상환 요청 → 토큰 잠금 →
운영사 판단 → 인도 기관 인계 → 완료 확인 → 소각으로 **구간이 셋**이라, 「누구의 시계에서 멈췄는가」가
가장 선명하게 갈립니다. 토큰 잠금으로 운영사 시계가, 인계로 인도 기관 시계가 시작됩니다.
`RedemptionInbox`는 보유자 서명 요청부터 거래소 전달·거절·무응답까지 추적합니다.

**에스크로는 자산 종류를 모릅니다.** `RedemptionEscrow`가 받는 것은 소각 가능한 임의의
ERC20(`IBurnableERC20`)이고 `MockGold`는 시연용 목업입니다. 다른 자산으로 옮기는 데 계약 재작성은
필요하지 않습니다. 금을 고른 근거와 그 맞교환은 [1절](#1-문제-정의--누가-이걸-쓰는가)과 한계 16번에
적었습니다.

금 상환 확장은 포크 [tnwjd023-boop/BlockNotice_GoldRWA](https://github.com/tnwjd023-boop/BlockNotice_GoldRWA)에서
개발해 이 저장소 `main`에 병합했습니다. 대회 기간 안의 작업입니다.
...
**1차 대상: 자기 키로 서명할 수 있고, 기관에 증빙을 요구할 힘이 있는 요청자.**
요청자가 키를 쥐고 있어야 영수증이 그 사람에게 묶이고, 계약에 기한이 있어야 「기한 내 증빙하지
않았다」가 확정됩니다. 둘 중 하나라도 없으면 이 구조는 값을 못 합니다. 자산 종류는 조건이 아닙니다.

**첫 장면으로 금 RWA 상환을 골랐습니다.** 잠금은 온체인이어도 운영사의 승인·제한 판단, 인도 기관에
...
**이 저장소에 대회 前 코드는 없습니다.** 첫 커밋이 2026-09-13이고 빌드 기간은 2026-09-07 ~ 09-20입니다.
문서에 나오는 「`39139b9`에서 확장」은 대회 기간 안의 앞 단계를 가리킵니다 — 영수증 경로를 먼저 만들고,
그 위에 금 RWA 상환(`RedemptionEscrow`·`RedemptionInbox`·인박스 연결 검증·배포 대조·시나리오 12개 추가)을
올린 순서입니다. 참고한 외부 라이브러리는 `forge-std`뿐이고 컨트랙트·검증기·시나리오는 전부 이번에 작성했습니다.
...
제출 전 5분 발표 대본은 [`DEMO.md`](DEMO.md)입니다.

피치덱은 12장짜리 두 벌이고 **같은 디자인 시스템**을 씁니다.

| 덱 | 생성기 | HTML |
|---|---|---|
| [한글 PDF](deck/BlockNotice_GoldRWA_pitch.pdf) | [`pitch.ko.source.cjs`](deck/pitch.ko.source.cjs) | [`pitch.ko.html`](deck/pitch.ko.html) |
| [영문 PDF](deck/BlockNotice_GoldRWA_pitch.en.pdf) | [`pitch.en.source.cjs`](deck/pitch.en.source.cjs) | [`pitch.en.html`](deck/pitch.en.html) |
...
[`deck/BlockNotice_pitch.pdf`](deck/BlockNotice_pitch.pdf)는 에스크로·인박스 확장 이전 자료입니다.
...
MIT가 아니라 Apache-2.0을 고른 이유는 **특허 실시권 조항**입니다. 이 원장은 기관이
서명한 약속을 다루는 코드라, 도입을 검토하는 쪽 법무가 특허 리스크를 먼저 봅니다.
Apache-2.0은 기여자가 자기 특허에 대한 실시권을 함께 준다고 명시하고, 특허 소송을
제기하면 그 권리가 종료된다는 조항까지 둡니다. 기업 상대 검토에서 기대되는 형태입니다.
